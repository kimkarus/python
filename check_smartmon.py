...
_smartctl_path = "C:/smartmontools/bin/smartctl.exe"
#_smartctl_path = "C:/Program Files/smartmontools/bin/smartctl.exe"
...
def check_device_permissions(path, level):
    ...
    """
    vprint(3, "Check if %s does exist and/or can be read" % path)
    massege = ""
    error = 0
    if not os.access(path, os.F_OK):
        massege = "UNKNOWN: no such device found"
        error = 3
    elif not os.access(path, os.R_OK):
        massege = "UNKNOWN: no read permission given"
        error = 3
    else:
        return (0, "")
    if level > verbosity:
...
def parse_output(output, warning_temp, critical_temp):
    ...
    # check health status
    health_status_str = "OK"
    while health_status not in ["PASSED", "OK"]:
        return_status = 2
        health_status_str = "ERROR"
...
if __name__ == "__main__":
    ...
    for device in devices:
        ...
        (return_status, message) = check_device_permissions(device, 1)
        if return_status != 0:
        ...
        return_status, output, message = call_smartmontools( _smartctl_path, device)
        if return_status != 0:
            if exit_status < return_status:
                exit_status = return_status
            return_text += message
        if output != "":
            ...
            return_status, device_status = parse_output(output, options.warning_temp, options.critical_temp)
            if exit_status < return_status:
                exit_status = return_status
